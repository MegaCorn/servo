     ...
     $length_and_data: ident) => {
        ...
        impl TypedArrayElement for $t {
            ...
            unsafe fn length_and_data(obj: *mut JSObject) -> (*mut Self::Element, usize) {
                ...
                $length_and_data(obj, &mut len, &mut shared);
                assert!(!shared);
                (data, len)
            }
        }
    ...
    ($t: ident,
     ...
     $create_new: ident) => {
        ...
        impl TypedArrayElementCreator for $t {
            ...
            unsafe fn get_data(obj: *mut JSObject) -> *mut Self::Element {
                let mut shared = false;
                // let data = $get_data(obj, &mut shared);
                // assert!(!shared);
                // data
                ptr::null_mut()
            }
...
fn mock_unwrap_array(maybe_wrapped: *mut JSObject) -> *mut JSObject {
    ptr::null_mut()
}

fn mock_get_array_length_and_data(
    obj: *mut JSObject,
    length: *mut usize,
    is_shared_memory: *mut bool,
) {}

fn mock_new_array(cx: *mut JSContext, nelements: usize) -> *mut JSObject {
    ptr::null_mut()
}
...
typed_array_element!(
    ...
    u8,
    mock_unwrap_array,
    mock_get_array_length_and_data,
    mock_new_array
);
typed_array_element!(
    Uint16,
...
unsafe impl<T: TypedArrayElement> js::gc::Traceable for TypedArray<T, Box<Heap<*mut JSObject>>> {
