...
impl Window {
    ...
    pub(crate) fn new(
        ...
        unsafe {
            ...
            let global_raw = DomRoot::from_ref(root.as_global_scope());
...

            {
                ...
                let fn_setTimeout = v8::Function::new(
                    ...
                    mut rv: v8::ReturnValue<v8::Value>| {
                        ...
                        let mut func = Function::new_v8(JSContext::from_ptr(std::ptr::null_mut()), std::ptr::null_mut(), global_, global_scope.value.ptr.as_ptr() as *mut js::jsapi::JSObject);
                        let arg1 = StringOrFunction::Function(func);
                        let arg2 = args.get(1).int32_value(scope).unwrap();
                        let arg3 = vec![];
                        let ret_ = unsafe { (*raw).SetTimeout(arg0, arg1, arg2, arg3) };

                        let ret = v8::Integer::new(scope, ret_);
                        rv.set(ret.into());
                    },
                ).unwrap();
                v8_global_add_fn!(scope, context, fn_setTimeout, setTimeout);

                let fn_clearTimeout = v8::Function::new(
                    scope,
                    |scope: &mut v8::HandleScope,
                    args: v8::FunctionCallbackArguments,
                    mut rv: v8::ReturnValue<v8::Value>| {
                        ...
                        let value: v8::Local<v8::External> = data.try_into().unwrap();
                        let raw = value.value() as *const crate::dom::types::Window;
                        let arg0 = args.get(0).int32_value(scope).unwrap();
                        unsafe { (*raw).ClearTimeout(arg0) };

                    },
                ).unwrap();
                v8_global_add_fn!(scope, context, fn_clearTimeout, clearTimeout);

                let fn_getComputedStyle = v8::Function::new(
                    scope,
                    |scope: &mut v8::HandleScope,
                    args: v8::FunctionCallbackArguments,
                    mut rv: v8::ReturnValue<v8::Value>| {
                        ...
                        // let arg0 = value0.value() as *const crate::dom::types::Element;
                        // let arg0 = unsafe {&*arg0};

                        // let ret = unsafe { (*raw).GetComputedStyle(arg0, None)};
                ...
                v8_global_add_fn!(scope, context, fn_getComputedStyle, getComputedStyle);
            }
    ...
    pub(crate) fn new_URLSearchParams(&self, url: String) -> DomRoot<URLSearchParams> {
...
pub fn URLSearchParams_callback(
    ...
    if (args.length() > 0) {
        ...
        let ret = unsafe { (*raw).new_URLSearchParams(String::new()) };
        let template = ret.new_template(scope);
        template.set_internal_field_count(1);
        let object = template.new_instance(scope).unwrap();
        let raw_ = ret.value.ptr.as_ptr() as *mut std::ffi::c_void;
