...
impl Document {
    ...
    pub(crate) fn add_deferred_script(&self, script: &HTMLScriptElement) {
        println!("jinguoen add_deferred_script");
    ...
    pub(crate) fn deferred_script_loaded(&self, element: &HTMLScriptElement, result: ScriptResult) {
        println!("jinguoen deferred_script_loaded");
    ...
    fn process_deferred_scripts(&self) {
        println!("jinguoen process_deferred_scripts");
        if self.ready_state.get() != DocumentReadyState::Interactive {
            println!("jinguoen process_deferred_scripts 1");
            return;
        }
        // Part of substep 1.
        loop {
            if self.script_blocking_stylesheets_count.get() > 0 {
                println!("jinguoen process_deferred_scripts 2");
                return;
            }
            if let Some((element, result)) = self.deferred_scripts.take_next_ready_to_be_executed()
            {
                println!("jinguoen process_deferred_scripts 3");
                element.execute(result);
            } else {
                println!("jinguoen process_deferred_scripts 4");
                break;
            }
        }
        if self.deferred_scripts.is_empty() {
            println!("jinguoen process_deferred_scripts 5");
